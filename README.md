...
- **Automated OSINT pipelines** — multi-stage LLM extraction, entity disambiguation, convergence detection, rhetoric vs. reality analysis, web-grounded fact verification
...
### 🛒 OSINT Intelligence Pipeline — Now Available
...
- Nine-tab live dashboard on GitHub Pages — no servers, no Docker, no databases
- Self-verifying output — the pipeline fact-checks itself against live web evidence before publishing
- Prediction tracking with full audit trail — confirmed and failed forecasts documented transparently
- Fully configurable — swap every tracked topic, signal, and entity rule from a single JSON config
